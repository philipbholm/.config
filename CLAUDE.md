...
- `dev/` — Development helper scripts symlinked to `~/bin/`. Core scripts:
  ...
  - `wt-down.sh` — Tear down the worktree you are standing in: nuke its dev stack, remove the directory, re-sync context. Refuses on a dirty tree and never deletes the branch (unlike the retired `gwd`, which discarded uncommitted work). There is no creation counterpart — worktrees are created natively by Claude Code's `EnterWorktree`.
