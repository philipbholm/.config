...
- `claude/` — Claude Code config: settings (model, permissions, plugins, hook declarations), custom agents, custom skills, `statusline-command.sh`, and `bin/` helpers. Both agents and skills are split by profile: `claude/skills/` holds the profile-agnostic skills (explain-diff-html, fix-pr-feedback) and `claude/skills.work/` the Ledidi-specific ones (create-issue, plan, implement, code-review, fix-feedback, learn, review-pr, distribute); `claude/agents/` is the profile-agnostic agent dir (currently empty) and `claude/agents.work/` holds the Ledidi reviewers (ledidi-code-reviewer, ledidi-security-auth-reviewer, ledidi-test-reviewer). Hook *scripts* live in `dev/` (see `dev/claude-notify.sh`).
