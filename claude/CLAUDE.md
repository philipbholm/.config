...
Repos under `~/work/` keep their agent issue tracker **outside** the repo, at:

    ~/work/.scratch/<repo-name>/agents/issue-tracker.md
...
effort:
    ...
    ~/work/.scratch/<repo-name>/<effort>/

The path is outside the repo on purpose. Nothing there is committed or shared.
So no file in the repo points at it, and no search of the repo finds it.
...
If `~/work/.scratch/<repo-name>/` does not exist, the repo has no tracker yet.
