...
No dev stack is running for this worktree, so it has no ports yet. Start only what the task needs: `dev stack up postgres -d` for the backend suite, `dev stack up` for browser work or E2E. Either one fills in this table.
'
...
No dev stack is running for the main checkout, so it has no ports yet. `dev stack up`
...
# stack.sh writes a slot file for worktrees only, so the main checkout's stack has
# to be read out of Docker. Slot 0 is its number, in stack.sh's main mode and in
...
[[ -d "$MAIN_REPO" ]] || dev_cli_error "Ledidi checkout not found: $MAIN_REPO"
...
add_target() {
  ...
  if [[ "$target" == "$MAIN_REPO" ]]; then
    ...
    if [[ -f "$slot_file" ]]; then
      targets+=("$target:$(tr -d '[:space:]' < "$slot_file")")
    else
      targets+=("$target:$NO_STACK")
    fi
  fi
}
...
if [[ "$all_worktrees" == true ]]; then
  while IFS= read -r target; do
    [[ -d "$target" ]] && add_target "$target"
  done < <(git -C "$MAIN_REPO" worktree list --porcelain | sed -n 's/^worktree //p')
else
  current_repo=$(git rev-parse --show-toplevel 2>/dev/null) ||
    dev_cli_error "not inside a Ledidi checkout"
  common_dir=$(dev_git_common_dir_abs "$current_repo")
  [[ "$(dirname "$common_dir")" == "$MAIN_REPO" ]] ||
    dev_cli_error "current repository is not a Ledidi checkout"
  add_target "$current_repo"
fi
...
if [[ "$duplicate_found" == true ]]; then
  echo "Warning: run 'dev stack up' in a conflicting worktree to reassign its saved slot." >&2
fi
