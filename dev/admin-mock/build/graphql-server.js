...
import { allUsers, findByUserName, findByIds, } from "./users.js";
...
function buildMainServer(fastify) {
    const typeDefs = gql(readSchema("admin.graphql") +
    ...
    const federatedSchema = rawSchema.replace("type User {", 'type User @key(fields: "id") {');
  ...
  `;
    const resolvers = {
        Query: {
            ...
            findUserIdentityByUserId: (_, { userId }) => ({
                userId,
                userIdentity: {
                    id: userId,
...
function buildPublicServer(fastify) {
...
export async function startGraphQLServer(port) {
