...
shift
case "$subcommand" in
    up|down|destroy|list|logs|exec|ps|build|stop|restart|config|pull|images|top|events|port|run) ;;
    *) dev_cli_error "unknown stack action: $subcommand" ;;
esac
if [[ "$subcommand" == list && $# -gt 0 ]]; then
    dev_cli_error "dev stack list takes no arguments"
fi
if [[ "$subcommand" == destroy && $# -gt 0 && ( $# -ne 1 || "$1" != --yes ) ]]; then
    dev_cli_error "use dev stack destroy [--yes]"
fi
...
SYNC_CONTEXT="$SCRIPT_DIR/context-render.sh"
...
# e.g. `dev stack up` in some other git repo. Name that instead of letting an empty
...
require_services() {
    ...
    echo "Pass the services you want explicitly: dev stack $subcommand <service>..." >&2
    exit 1
}
...
detect_checkout() {
    repo_root=$(git rev-parse --show-toplevel 2>/dev/null) ||
        dev_cli_error "not inside a Git checkout"
    base_project_name=$(dev_checkout_id_for_repo "$repo_root")
    ...
    case "$tmp_dir" in
        ''|'/'|"$HOME"|"$HOME/") dev_cli_error "refusing to use '$tmp_dir' as this stack's state dir" ;;
    esac
...
is_slot_in_use() {
    ...
    # (containers were removed manually instead of via `dev stack destroy`).
...
slot_from_existing_stack() {
    ...
    container=$(docker ps -aq \
        --filter "label=${DEV_CHECKOUT_LABEL}=${base_project_name}" \
...
resolve_slot() {
    ...
    # No containers exist for this checkout. For `up`, re-allocate the lowest free
    ...
    if [ -n "$saved" ]; then
        if ! is_slot_in_use "$saved"; then
            ...
            echo "Use 'dev stack up --slot N' to specify a different slot." >&2
            exit 1
        fi
    fi

    echo "Error: No slot assigned yet. Run 'dev stack up' first." >&2
    exit 1
}
...
# Auto-generated by stack.sh (main mode)
...
services:
  registries-frontend:
    labels:
      ${DEV_SLOT_LABEL}: "0"
      ${DEV_CHECKOUT_LABEL}: "${base_project_name}"
  admin:
    profiles: ["disabled"]
  mysql:
    profiles: ["disabled"]
...
generate_worktree_override() {
    ...
    # service below. Baked in as a literal so every recreate path (stack.sh,
    ...
    cat > "$override_file" <<YAML
# Auto-generated by stack.sh (worktree slot $s, offset $offset)
# Do not edit — regenerated on each run.
...
# Stack startup knows the allocated slot before it can be read from Docker.
# Both renderers fill the port table through dev_apply_context_ports.
sync_context_files() {
    local s=$1
...
restore_context_files() {
    ...
    (cd "$repo_root" && bash "$SYNC_CONTEXT") || echo "Warning: context render failed; CLAUDE.local.md and AGENTS.md may be stale." >&2
}
...
if ! [[ "$resolved_slot" =~ ^[0-9]$ ]]; then
    ...
    echo "Fix or delete $slot_file, then run 'dev stack up'." >&2
    exit 1
fi
...
case "$subcommand" in
    ...
    destroy)
        ...
        if [ "$destroy_confirmed" = "true" ]; then
        ...
        elif [ "$destroy_confirmed" != "true" ]; then
            ...
            echo "Re-run as 'dev stack destroy --yes' if you really mean to remove the volumes and images." >&2
            exit 1
        fi
        generate_override "$resolved_slot"
