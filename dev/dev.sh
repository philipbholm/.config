...
generate_router_config() {
    local source="$1"
    local target="$2"
    local frontend_port="${3:-3001}"

    awk -v port="$frontend_port" '
    /^  match_origins:/ {
        in_origins = 1
        print "  policies:"
        print "    - match_origins:"
    ...
    {
        in_origins = 0
        if (/http:\/\/localhost:3001/)
            gsub(/http:\/\/localhost:3001/, "http://localhost:" port)
        print
    }
    ' "$source" > "$target"
}
...
generate_worktree_override() {
    ...
    generate_router_config \
        "$repo_root/services/apollo-router/router.docker.yaml" "$router_config" "$(( 3001 + offset ))"
