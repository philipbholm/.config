...
generate_router_config() {
    local source="$1"
    local target="$2"
    local frontend_port="${3:-$FRONTEND_BASE_PORT}"

    awk -v port="$frontend_port" -v base_port="$FRONTEND_BASE_PORT" '
    /^  match_origins:/ {
        in_origins = 1
        print "  policies:"
        print "    - match_origins:"
    ...
    {
        in_origins = 0
        pat = "http:\\/\\/localhost:" base_port
        if ($0 ~ pat)
            gsub(pat, "http://localhost:" port)
        print
    }
    ' "$source" > "$target"
}
...
generate_main_override() {
    ...
    local router_config="$tmp_dir/router.docker.main.yaml"
...
services:
  ...
  router:
    ...
    volumes: !override
      ...
      - $tmp_dir/router.docker.main.yaml:/dist/config/router.yaml
  registries:
    networks:
      - default
      - $ADMIN_MOCK_NET
...
generate_worktree_override() {
    ...
    generate_router_config \
        "$repo_root/services/apollo-router/router.docker.yaml" "$router_config" "$(( FRONTEND_BASE_PORT + offset ))"
...
default_services=$(available_services "registries-frontend router router-autoupdate postgres codelist registries agent")
